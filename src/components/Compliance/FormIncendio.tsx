...
export function FormIncendio({ onSubmit, loading }: FormIncendioProps) {
    const config = COMPLIANCE_FIELDS.incendio;
    const { dados, handleChange } = useFormState({
        risco: 'leve',
        numero_hidrantes: 2,
        vazao_minima_litros: 100
    });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm">
                ...
                <div className="mt-6 flex justify-end">
                    ...
                    >
                        {loading ? 'Validando...' : 'Validar Hidrantes'}
                    </button>
                </div>
            </div>
        </form>
