...
interface FormEsgotoProps {
    onSubmit: (dados: Record<string, unknown>) => void;
    loading: boolean;
}

export function FormEsgoto({ onSubmit, loading }: FormEsgotoProps) {
    const config = COMPLIANCE_FIELDS.esgoto;
    const { dados, handleChange } = useFormState({
        diametro_mm: 100, // Default sensible
        vazao_ls: 2.0,
        declividade_percent: 1.0,
        material: 'pvc'
    });
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="bg-white p-6 border border-slate-200 rounded-lg shadow-sm">
                ...
                <div className="mt-6 flex justify-end">
                    ...
                    >
                        {loading ? 'Validando...' : 'Validar Sistema'}
                    </button>
                </div>
            </div>
        </form>
